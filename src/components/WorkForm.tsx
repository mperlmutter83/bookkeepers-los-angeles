...
const inputClass =
  "w-full border border-gray-300 rounded-lg px-4 py-3 text-navy focus:outline-none focus:ring-2 focus:ring-accent";
...
export default function WorkForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const details = `Experience: ${formData.experience || "not specified"}. ${formData.message}`.trim();
    ...
    try {
      const response = await fetch("/api/lead", {
        ...
        body: JSON.stringify({
          ...
          service_needed: `Service: Looking for Work | Details: ${details} | Source: bookkeeperslosangeles.com`,
      ...
      if (response.ok) {
        // GA4/GTM conversion event — picked up by the site's GTM container.
  ...
  if (status === "success") {
    return (
      <div className="text-center">
        ...
        <h3 className="mt-3 font-heading text-2xl font-bold text-navy">You&apos;re on the list!</h3>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label htmlFor="experience" className="block font-heading font-semibold mb-1 text-navy">
          Experience
        </label>
        <select
          id="experience"
      ...
      <div>
        ...
        <textarea
          id="message"
          rows={3}
          placeholder="e.g. Available weekdays, QuickBooks certified, looking for full-time…"
